...
class Scene:
    def __init__(self, init_randomly=False, map_height=10, map_width=10, block_size=15):
    ...
    def move(self, action):
        ...
        in_a_loop = (self.last_food_eaten > self.width * self.height) and (self.food_eaten < 15)
        
        # The snake collided with itself or move in the opposite direction or collided with the wall
        # or is in a loop or something like that
        if (not valid_move) or head_x < 0 or head_x >= self.width or head_y < 0 or head_y >= self.height or in_a_loop:
            done = True
            reward = -40
    ...
    def new_food_position(self):
        possible_food_positions = [(x, y) for x in range(self.width) for y in range(self.height) if (x, y) not in self.snake.body]
    ...
    def draw(self):
        ...
        for block in self.snake.body:
            pygame.draw.rect(self.screen, (0, 0, 0), (block[0] * self.block_size, block[1] * self.block_size, self.block_size, self.block_size))
            
        # Draw the food
        pygame.draw.rect(self.screen, (255, 0, 0), (self.food_position[0] * self.block_size, self.food_position[1] * self.block_size, self.block_size, self.block_size))
    ...
    def state(self):
        ...
        right = 1 if head_x == self.width - 1 or (head_x + 1, head_y) in self.snake.body[:-1] or (head_x + 1, head_y) == (prev_head_x, prev_head_y) else 0
        down = 1 if head_y == self.height - 1 or (head_x, head_y + 1) in self.snake.body[:-1] or (head_x, head_y + 1) == (prev_head_x, prev_head_y) else 0
        ...
        relative_food_direction = np.array(self.food_position) - np.array([head_x, head_y])        
        ...
        # return (matrix_one_hot, obstacles, no_body_blocks, relative_food_direction)
